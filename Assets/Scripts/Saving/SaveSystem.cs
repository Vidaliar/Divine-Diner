...
public class SaveSystem : MonoBehaviour
{
    ...
    public SaveMeta GetMeta(string profile, int slotIndex)
    {
        ...
        {
            Debug.LogWarning($"[SaveSystem] 读取 Meta 失败：{path}\n{e}");
            return default;
        }
    }
    ...
    private IEnumerator SaveRoutine(string profile, int slotIndex, string title, string subtitle)
    {
        ...
        if (captureViaCamera)
        {
            if (captureCamera == null) Debug.LogWarning("[SaveSystem] captureViaCamera=true 但未指定 captureCamera");
        ...
        {
            title = string.IsNullOrEmpty(title) ? $"Day {data.day}" : title,
        ...
        Debug.Log($"[SaveSystem] Saved：{profile}/slot{slotIndex}");
    }
